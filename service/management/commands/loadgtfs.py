...
class Command(BaseCommand):
    ...
    def _load_calendar_dates(self, root_dir):
        ...
        def create_cmd(line):
            ...
            return CalendarDate.objects.get_or_create(
                ...
                date=date,
                exception_type=ExceptionType.objects.get(
                    value=check_field(line, 'exception_type')))

        self._load(root_dir, "calendar_dates.txt", create_cmd, fields,
                   optional=True)
    ...
    def _load_calendar(self, root_dir):
        ...
        def create_cmd(line):
            ...
            return Calendar.objects.get_or_create(service=Service.objects.get(
                service_id=check_field(line, 'service_id')),
                                                  monday=check_field(line,
                                                                     'monday'),
                                                  tuesday=check_field(line,
                                                                      'tuesday'),
                                                  wednesday=check_field(line,
                                                                        'wednesday'),
                                                  thursday=check_field(line,
                                                                       'thursday'),
                                                  friday=check_field(line,
                                                                     'friday'),
                                                  saturday=check_field(line,
                                                                       'saturday'),
                                                  sunday=check_field(line,
                                                                     'sunday'),
    ...
    def _load_stop_times(self, root_dir):
        ...
        def create_cmd(line):
            (hour, minute, sec) = map(int,
                                      check_field(line, 'arrival_time').split(
                                          ":"))
            arrival_time = time(hour % 24, minute % 60, sec % 60)
            (hour, minute, sec) = map(int,
                                      check_field(line, 'departure_time').split(
                                          ":"))
            ...
            if check_field(line, 'pickup_type', optional=True):
                stop.pickup_type = PickupType.objects.get(
                    value=check_field(line, 'pickup_type'))

            # check drop off type :
            if check_field(line, 'drop_off_type', optional=True):
                stop.drop_off_type = DropOffType.objects.get(
                    value=check_field(line, 'drop_off_type'))
            return (stop, created)

        self._load(root_dir, "stop_times.txt", create_cmd, fields,
                   optional=False)
        ...
        def create_cmd(line):
            (stop, created) = Stop.objects.get_or_create(
                ...
                geopoint=fromstr("POINT(%s %s)" %
                                 (float(check_field(line, 'stop_lat')),
                                  float(check_field(line, 'stop_lon')))))
            # create zone
            ...
            if check_field(line, 'parent_station', optional=True):
                try:
                    stop.parent_station = Stop.objects.get(
                        stop_id=check_field(line, 'parent_station',
                                            optional=True))
                except:
                    pass
            return (stop, created)
    ...
    def _load_routes(self, root_dir):
        ...
        def create_cmd(line):
            (route, created) = Route.objects.get_or_create(
                ...
                route_type=RouteType.objects.get(
                    value=check_field(line, 'route_type')))
            # link related agency
            if check_field(line, 'agency_id', optional=True):
                try:
                    route.agency = Agency.objects.get(
        ...
        def create_cmd(line):
            (trip, created) = Trip.objects.get_or_create(
                ...
                service=Service.objects.get_or_create(
                    service_id=check_field(line, 'service_id'))[0],
                trip_id=check_field(line, 'trip_id'))
            # link related direction_id
            if check_field(line, 'direction_id', optional=True):
                trip.direction = Direction.objects.get(
                    value=check_field(line, 'direction_id'))
            if check_field(line, 'block_id', optional=True):
                trip.block = Block.objects.get_or_create(
                    block_id=check_field(line, 'block_id'))[0]
            return (trip, created)
    ...
    def _load_shapes(self, root_dir):
        ...
        def create_cmd(line):
            geopoint = fromstr("POINT(%s %s)" % (
                float(check_field(line, 'shape_pt_lat')),
                float(check_field(line, 'shape_pt_lon'))))
    ...
    def _load(self, root_dir, filename, get_or_create_cmd, fields,
              optional=False):
        count = 0
        try:
            ...
            for line in reader:
                ...
                for key, value in fields:
                    if not obj.__dict__[key] and check_field(line, value,
                                                             optional=True):
                ...
                if count % 10000 == 0:
                    self.stdout.write(
                        "\tLoading %s lines from filename %s, still in progress...\n" % (
                            count, filename))
            ...
            if not optional:
                raise CommandError(
                    "Could not load %s data properly, failed at line %s. Fix the following problems, this file is required : " % (
                        filename, count) + str(e))
            else:
                self.stdout.write(
                    "Warning file %s not found or not properly loaded.\n" % filename)
